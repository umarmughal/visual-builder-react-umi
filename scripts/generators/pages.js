...
const cardTpl = (items) => {
  ...
  items &&
    items.forEach((card) => {
      cards =
        cards +
        `<div className="${card.extraClass ? card.extraClass : 'card'}">${widgetTpl(
          card.widgets,
        )}</div>`
    })
  return cards
...
const widgetTpl = (items) => {
  ...
  items &&
    items.forEach((widget) => {
      ...
      const props = widget.data ? ` data={${JSON.stringify(widget.data).replace(/"/gi, "'")}}` : ''
      let widgetCode = ''
      if (widget.wrapper === false) {
        widgetCode = `<${folder}${name}${props} />`
      } else {
        widgetCode = `<div className="${
          widget.wrapper ? widget.wrapper : 'card-body'
        }"><${folder}${name}${props} /></div>`
      }
...
module.exports = (config, content) => {
  ...
  const foldersToRemove = GET_DIR_FOLDERS(pagesFolder).filter((item) => {
    return item !== 'auth'
  })
          ...
          content.forEach((row) => {
            row.columns &&
              row.columns.forEach((columns) => {
                columns.cards &&
                  columns.cards.forEach((cards) => {
                    cards.widgets &&
                      cards.widgets.forEach((widget) => {
                        widget && widgetsList.push(widget.name)
                      })
                  })
              })
          })
        const filteredWidgetsList = widgetsList.filter(
